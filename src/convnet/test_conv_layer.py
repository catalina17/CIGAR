...
class TestConvLayer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.layer._filter_weights = np.ones((2, 2, 3), dtype=np.float64)
        self.layer._filter_weights[1, :, :] /= 2.0

        self.layer_cuda = ConvLayerCUDA(2, (2, 2), 1, padding_mode=False)
        self.layer_cuda.set_input_shape((2, 8))
        self.layer_cuda._filter_weights = np.ones((2, 2, 3), dtype=np.float64)
        self.layer_cuda._filter_weights[1, :, :] /= 2.0

        self.input = np.array([[2, 2, 2, 2, 2, 2, 2, 4],
                               [4, 4, 4, 4, 4, 4, 4, 8]], dtype=np.float64)

    def test_forward_prop(self):
        expected_output = np.array([[18, 18, 18, 18, 18, 24],
                                    [9, 9, 9, 9, 9, 12]], dtype=np.float64)
    ...
    def test_back_prop(self):
        ...
        out_grad = np.ones((2, 6), dtype=np.float64)
